...
import { extractRecords, formatINR, getDocumentAmount, getDocumentDate } from "../utils";
...
export function useDashboardAnalytics(): DashboardAnalytics {
  ...
  const result = useMemo(() => {
    ...
    const dateMap = new Map<string, { invoiced: number; paid: number; current: number; overdue: number }>();
    ...
    filteredDocs.forEach((doc) => {
      ...
      const paidAmt = typeof doc.paid_amount === "number" ? doc.paid_amount : status === "paid" ? amount : 0;
      const isOverdue = status === "overdue" || (doc.due_date && new Date(String(doc.due_date)) < now && status !== "paid");
      ...
      const existing = dateMap.get(dKey) || { invoiced: 0, paid: 0, current: 0, overdue: 0 };
      dateMap.set(dKey, {
    ...
    if (filteredDocs.length === 0) {
      ...
      const sampleInvoicedPattern = [120000, 450000, 1398522, 2100000, 850000, 4800000, 2600000, 18700000];
      const samplePaidPattern = [50000, 100000, 0, 800000, 300000, 1500000, 900000, 36299828 / 5];

      chartData = Array.from({ length: numPoints }).map((_, i) => {
        const pointDate = new Date(cutoffDate.getTime() + i * stepDays * 24 * 60 * 60 * 1000);
    ...
    const statusData: StatusPiePoint[] = [
      {
        ...
        percentage: totalStatusSum > 0 ? Math.round((totalPaid / totalStatusSum) * 100) : 0,
      },
      {
        name: "Current Pending",
        value: totalCurrent,
        color: "#10b981", // Green swatch
        percentage: totalStatusSum > 0 ? Math.round((totalCurrent / totalStatusSum) * 100) : 0,
      },
      {
        name: "Overdue",
        value: totalOverdue,
        color: "#ef4444", // Red swatch
        percentage: totalStatusSum > 0 ? Math.round((totalOverdue / totalStatusSum) * 100) : 0,
      },
    ];
    ...
    const hasRealDocs = rawSalesInvoices.length > 0 || rawPurchaseInvoices.length > 0;
    const monthlyComparison: MonthlyComparisonPoint[] = monthNames.slice(0, 9).map((m, idx) => {
      ...
      return {
        ...
        purchases: Math.round(2200000 + Math.cos(idx) * 1100000 + idx * 500000),
      };
    });
