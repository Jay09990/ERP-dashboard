...
export function UserPermissionMatrixModal({
  userId,
  userName,
  roleId,
  onClose,
  ...
  useEffect(() => {
    if (allPermissions) {
      ...
      const merged = allPermissions.map((p: PermissionItem) => {
        ...
        const effectiveAllowed = hasUserOverride ? !!userAllowed : roleAllowed;
        const isInherited = !hasUserOverride && roleAllowed;

        return {
          ...p,
          is_allowed: effectiveAllowed,
    ...
    setPermissions((prev) =>
      prev.map((p) =>
        p.permission_name === permissionName
          ? { ...p, is_allowed: !p.is_allowed, inherited: false }
          : p,
      ),
    );
  };
    ...
    setPermissions((prev) =>
      prev.map((p) =>
        (p.module_name || "General") === moduleName
          ? { ...p, is_allowed: enable, inherited: false }
          : p,
      ),
    );
  };
  ...
  // Cache grouped permissions by module to avoid repeated O(N) array filter scans during module rendering and search filter
      ...
      <div className="altrex-dialog-backdrop">
        <div className="altrex-dialog">
          <div className="altrex-table-state">
            ...
            <span>Loading user & role permissions...</span>
          </div>
        </div>
      </div>
    );
              ...
              {activeModuleItems.length === 0 ? (
                ...
                >
                  {activeModuleItems.map((item) => {
                    ...
                    const chip = actionColors[action] || {
                    ...
                    return (
                      ...
                      >
                        ...
                        {item.is_allowed && item.inherited && (
                          <span
                            style={{
                              fontSize: "10px",
                              fontWeight: 600,
