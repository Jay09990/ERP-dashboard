...
export function LocationCascadeSelect({
  ...
  const { data: countriesData, isLoading: areCountriesLoading } =
    countryApi.useList();
  ...
  return (
    <>
      <label className="altrex-field">
        ...
        >
          <option value="">
            {areCountriesLoading ? "Loading countries..." : "Select country..."}
          </option>
          {countries.map((country) => (
            <option
              key={toId(country.country_id ?? country.id)}
              value={toId(country.country_id ?? country.id)}
            >
              {String(
                country.country_name ?? country.name ?? "Unnamed country",
              )}
            </option>
          ))}
        </select>
      ...
      <label className="altrex-field">
        ...
        <select
          ...
          disabled={!currentCountry || areStatesLoading}
        >
          <option value="">
            {!currentCountry
          ...
          {states.map((state) => (
            <option
              key={toId(state.state_id ?? state.id)}
              value={toId(state.state_id ?? state.id)}
            >
              {String(state.state_name ?? state.name ?? "Unnamed state")}
            </option>
          ))}
        </select>
      </label>
      <label className="altrex-field">
        ...
        <select
          ...
          disabled={!currentState || areCitiesLoading}
        >
          <option value="">
            {!currentState
              ? "Select state first..."
          ...
          {cities.map((city) => (
            <option
              key={toId(city.city_id ?? city.id)}
              value={toId(city.city_id ?? city.id)}
            >
              {String(city.city_name ?? city.name ?? "Unnamed city")}
            </option>
          ))}
        </select>
      </label>
